---
title: "3.1 - Sampling Variability"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
The original sample proportions are $\hat{p}_{East}$ of 0.82 and $\hat{p}_{West}$ of 0.73. The difference between the proportions is about 0.09. 
...
If there was no association between the location and the drink preference, we could shuffle the data completely at random, and the result of that shuffle would be a potential observation that is as likely as the original observation. So let us shuffle the data and see what we get! 

As it turns out, by pure chance, the first shuffle of the drink variable gives the exact same summaries as the observed data! 
...
Now go to [this link](https://istats.shinyapps.io/Association_Categorical/){target="_blank" rel="noopener noreferrer"}, and fill in the contingency table as shown below. 

![](images/artofstats-01.jpg){width=40% }

###

```{r quiz-artofstats-1}
question("When looking at the descriptive statistics in the panel on the left, we can see that:",
  answer("Proportion of Cola for East is `0.824`, for West is `0.731` and the difference is `0.0928`", correct = TRUE, message = "Nice one!"),
  answer("Proportion of Cola for East is `0.731`, for West is `0.824` and the difference is `0.0928`"),
  answer("Proportion of Cola for East is `0.0928`, for West is `0.824` and the difference is `0.731`"),
...
You can now click on the `Permutation Distribution` tab on the top right hand side of the page.

![](images/artofstats-02.jpg){width=60% }

<br/>


In this panel, you can reshuffle the data (generate  permutations of the data). If the null hypothesis was true, there would be no association between the location and the drink preference. In that case, each permutation would be equally likely. 

You can now generate one permutation at a time, and see the result in the panel on the left. You can then increase the number of permutations to 100 at a time, and finally 10,000 permutations (it may take a few moments to complete, so you will need to be patient). You can now select the bin-width as shown below. 

<br/>

###

![](images/artofstats-03.jpg){width=60% }

<br/>

If you have done everything correctly, you should see the distribution of the permutations **assuming** that the null hypothesis is true, and there is no association between the location and the drink preference (see below).  

![](images/artofstats-04.jpg){width=80% }
...
```{r quiz-artofstats-2}
question("Assuming that the null hypothesis is true, what is the proportion of random shuffles that produce a difference that is more extreme that the one we observed (i.e., a difference that is greater than 0.0928):",
  answer("About 12%", correct = TRUE, message = "Nice one! We can see that the proportion of permutations, less than the Observed Difference (an observed difference of 0.0928) is about 88%. In other words, the probability of observing a more extreme difference is about 12%."),
  answer("About 88%"),
  answer("About 20%"),
  answer("About 9%"),
  allow_retry = TRUE, random_answer_order = TRUE
...
Throughout this lesson, you will use the [NHANES](https://CRAN.R-project.org/package=NHANES){target="_blank" rel="noopener noreferrer"} dataset from the NHANES R package. The data are collected by the Center for Disease Control (CDC, the national public health institute in the United States) and can be thought of as a random sample of US residents.
...
(1) Use the `names()` function to list the variables included in the `NHANES` data frame from the [NHANES](https://CRAN.R-project.org/package=NHANES){target="_blank" rel="noopener noreferrer"} package. (Click the link for more info.)
...
(2) Using the `NHANES` data, create a bar plot of `HomeOwn` (fill color) broken down by `Gender` (x-axis).  
...
Using the `NHANES` dataset, let's investigate the relationship between gender and home ownership. Remember, more information about the dataset can be found here: [NHANES](https://www.rdocumentation.org/packages/NHANES/topics/NHANES){target="_blank" rel="noopener noreferrer"}.
...
(1) From `NHANES`, select the `Gender` and `HomeOwn`ership columns, then filter the rows to keep individuals whose home ownership status is `"Own"` or `"Rent"`.  Save the result to the dataset named `homes`.
...
Now go to [this link](https://istats.shinyapps.io/Association_Categorical/){target="_blank" rel="noopener noreferrer"}, and fill in the contingency table according to the data shown above.  

![](images/artofstats-05.jpg){width=40% }


Now you can generate many permutations, just like you've done previously. You will have to adjust the bin-width and the range on the x-axis in order to see the histogram of the permutations properly.

![](images/artofstats-06.jpg){width=40% }



```{r quiz-artofstats-3}
quiz( caption = "", 
  question("What was the observed **difference** between home ownership among men and among women?",
    answer("About 0.78%", correct = TRUE, message = "Nice one! On the `Data Entry` tab we can see that the difference of proportion is 0.00783"),
    answer("About 1%"),
    answer("About 10%"),
    answer("More than 10%"),
    allow_retry = TRUE, random_answer_order = TRUE
  ), 
  question("Assuming that the null hypothesis was true, what is the proportion of random shuffles that produce a **difference** that is more extreme that the one we observed?",
    answer("About 20%", correct = TRUE, message = "Nice one! We can see that the proportion of permutations that are less than the Observed Difference (an observed difference of 0.00783) is about 80%. In other words, the probability of observing a more extreme difference is about 20%."),
    answer("About 88%"),
    answer("About 12%"),
    answer("About 9%"),
    allow_retry = TRUE, random_answer_order = TRUE
...
(2) Now use R to find the observed difference in proportions of men who own and women who own, by carrying out the following steps: 

-   step 1. Using the `homes` dataset, group by `Gender`.

-   step 2. Summarize the proportion of individuals who own a home. *Note that this will calculate proportions for both genders since you've grouped by the `Gender` variable in the line before it.*  You can calculate the proportion by finding the `mean()` of the observations that equal `"Own"` in the `HomeOwn` variable.

-   step 3. Finally, summarize the observed difference in proportions using the `diff()` function to calculate the difference in male and female home ownership.
...
```
 ...
 (1) Using the infer framework, step through specifying the null model and then performing 10 permutations to evaluate whether home ownership status (i.e. `HomeOwn`) differs between the `"female"` and `"male"` groups:
...
(1) Using the infer framework, step through specifying the null model and then performing 100 permutations to evaluate whether home ownership status (i.e. `HomeOwn`) differs between the `"female"` and `"male"` groups:
