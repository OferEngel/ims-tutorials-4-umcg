...
A new drug is offered to a group with risk of developing Coronary artery disease, and members of the group are followed for a number of years to see if they had an event. The results are shown in the table below.
...
quiz(
  caption = "",
  question(
    "Based only on the information provided in the table, choose the correct statements:", allow_retry = TRUE, random_answer_order = TRUE, 
    answer("Females taking the drug suffered worse health outcomes than females not taking the drug", correct = TRUE), 
    answer("Males taking the drug suffered worse health outcomes than males not taking the drug", correct = TRUE), 
    answer("Overall, those who took the drug suffered worse health outcomes than males not taking the drug"),
    answer("Overall, males suffered worse health outcomes than females", correct = TRUE)
    )
)
```
...
The data presents a puzzling situation: the average person in the sample appears to fair better if they do take the drug (only 18.3% heart attack) than if they do not take the drug (21.7% heart attack). But if we look at the male and female subgroups, their members tend to fair worse on average when taking the drug, on average.

Based on these data, would you advise the drug to be approved? Probably not!

Often when one mentions "a relationship between variables" we think of a relationship between just two variables, say a so called explanatory variable (aka predictor e.g., taking the drug yes/no), and response variable (aka outcome, e.g., health outcomes).
...
quiz(
  ...
  question(
    "V1 - An animal's daily caloric intake<br/>V2 - The animal's body mass.", allow_retry = TRUE,
    answer("V1 is the explanatory and V2 is the outcome"), 
    answer("V2 is the explanatory and V1 is the outcome", correct = TRUE)
    )
)
...
```{r quiz-group-by}
question("In the code chunk above, what does the `group_by` clause actually do? What would happen if your remove the `group_by` clause? ", allow_retry = TRUE, random_answer_order = TRUE,
    answer("The `group_by` does nothing, removing it from the code changes nothing.", message = "Nope, try again!"),
    answer("The `group_by` tells the `count` to show the admission rate separately for each group (Females and Males).", correct = TRUE),
    answer("Removing the `group_by` clause, the `count` to show the admission rate for the entire university.", correct = TRUE),
    answer("The `group_by` tells the `count` to show the admission rate for the entire university.", message = "Nope, try again!")
)
```
...
```{r quiz-gender}
quiz(
  question("Which of the following best describes the relationship between admission status and gender?", correct = "Correct! Let's recap what we've learned in the next section.", allow_retry = TRUE,random_answer_order = TRUE,
