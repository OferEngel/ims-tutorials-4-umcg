...
question_checkbox("Why not take a census?", 
         allow_retry = TRUE, 
...
quiz(
  question(
    ...
    correct = "Nice! Let's continue to the next exercise.", 
    allow_retry = TRUE,
...
quiz(
  question("Which approach would likely be the **least effective** for selecting the schools where the survey will be conducted?", 
           correct = "Nice job! This sampling strategy would be a bad idea because each neighborhood has a unique socioeconomic status. A good study would collect information about every neighborhood.", 
           allow_retry = TRUE,
...
To do so we group our data by state, then count the number of observations per state.
...
The resulting output shows that this sampling approach yielded data from less than _50_ states, some states are over-represented in our sample and some are under represented. The number of counties sampled varies from state to state.
...
A quick glimpse at the resulting data frame confirms that we indeed have _150_ observations in this sample. In the following code do the following: 
