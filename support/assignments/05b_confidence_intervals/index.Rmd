...
<span style="color: red;font-size: 14px;font-weight: bold;">DEADLINE FOR COMPLETING THIS LAB: 19th October (during the lab) </span>
...
In this lab, you will assume this $\pi=62\%$ is the very true population proportion. In reality, we cannot observe this number, but for the purpose of this lab we work under this assumption. We will then sample our data from our hypothetical population, demonstrating that sample proportions vary from sample to sample.
...
-   Load the `tidyverse`, `mosaic`, `broom` and `infer` package.
...
prop_test(~climate_believers, data = samp_1) |> 
  tidy()
...
10.  Using the app, experiment with different sample sizes and comment on how the widths of intervals change as sample size changes (increases and decreases).

11.  Finally, given a sample size (say, 60), how does the width of the interval change as you increase the number of bootstrap samples.
