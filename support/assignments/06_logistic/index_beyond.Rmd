---
title: 'BMLR LAB 2: Logistic regression - DRAFT'
output:
  html_document:
    theme: cerulean
    highlight: pygments
...
3.  You will now estimate the same measures from the previous question, but this time using two types of regressions: a **logistic regression** and a regular **linear regression**. The advantage of using the regression models is that they provide the standard errors and the confidence intervals (which we do not observe in the contingency table).  Show your code and present your results in a table adding the confidence intervals (see below). Explain your findings: what are the differences between the three approaches (using a contingency table as in the question above, using logistic regression and using linear regression)? What explains these differences (or lack thereof)? What are the advantages/drawbacks of using one approach over the other? 
...
# Using a linear regression, violating assumptions
...
You should be able to find the confidence intervals of your log odds ratio from the logistic regression easily, using the following code: 

```{r echo=TRUE, eval=FALSE}
library(broom)
mdl.glm |> tidy(conf.int = TRUE)
```

And the confidence interval of the odds ratio by exponentiating the appropriate values, or by running the following code:


```{r echo=TRUE, eval=FALSE}
library(broom)
mdl.glm |> tidy(conf.int = TRUE, exponentiate = TRUE)
```

But how will you calculate the confidence intervals of the risk ratio? Or the risk difference? 
...
-   For the comparison argument you can use a variety of contrast indicators. Use `lnor` to estimate the log odds ratio (and its standard error and confidence interval). Use `ratio` for the risk ratio and `difference` for the probability difference. Compare different approaches and see if you get similar results! 
