...
1. In the figure above, you can see two ways of using the select function. The first does not use a pipe, and the second uses a pipe. What is the difference in the output between these two methods? Which method is better? 
...
2. In your own words, try to describe what the pivot functions do, and why they are important. You may use the cheat sheet shown above or the help function in R to explore these functions.
...
3. Which of the three data sets above would be considered a "tidy" dataset? 
...
4. Now create a tidy dataset, based on the information shown above. CLUE: The new dataset will have three variables: `country`, `year` and `n`.
...
5. Let's try to re-create a small snippet of the `pollution` dataset. Now re-create the tidy form of this dataset. CLUE: Use three variables: `city`, `large`, `small`. 
...
6. Now use the `pivot_wider` function to transform the data-set automatically! Fill in the code below and run it. 
...
7. Load the `storms` dataset to separate the `date` variable into the three variables: `year`, `month` and `storm`. 
...
8. What do the variables `x`, `y` and `z` refer to? 
...
9. Use `ggplot()` to create a histogram of `price`. Use `binwidth = 1000` to set the width of each bar to 1000.  
...
11. Create another histogram that displays the distribution of `carat` in the data set using `binwidth = 0.01`. Add a vertical line that marks the median carat value by layering `geom_vline(xintercept = median(diamonds$carat))` on top of the plot. Where does the median carat value lie?
...
12. Create a scatter plot with `carat` on the x-axis and `price` on the y-axis.



13. In the previous plot, `geom_point()` plotted many points on top of each other, making it difficult for us to understand where most of the data lies. **Try using `geom_hex()` instead of `geom_point()` to make a more effective plot.**
...
14. Use `diamonds` to display median price and median carat by `color`. Your resulting tibble should have two columns: `color` and `median_price` and `median_carat`.
...
15. How many diamonds of each cut are present in `j_diamonds`? Re-create the following table displaying the answer:
