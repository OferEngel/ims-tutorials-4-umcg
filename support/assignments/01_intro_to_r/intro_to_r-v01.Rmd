...
As the labs progress, you are encouraged to explore and experiment as much as you can! Before we get to that stage, however, you need to build some basic fluency in `R`. First, we will explore the fundamental building blocks of `R` and RStudio: the RStudio interface, reading in data, and basic commands for working with data in `R`.
...
-   **Panel (1)** on the left is where the action happens. This panel is called the *console*. Every time you launch RStudio, it will have the same text at the top of the console telling you the version of R that you're running.
    +   Below that information is the *prompt*, indicated by the `>` symbol. As its name suggests, this prompt is really a request: a request for a command. Initially, interacting with `R` is all about typing commands and interpreting the output. 
...
:::{#boxedtext}
Before starting, we recommend that you configure your RStudio environment with the follwoing global options First, go to the menu point `Tools->Global Options`. Then make sure the following checkboxes are left **unchecked**. 
...
If one or both of these packages do not appear when you type in their name, install the missing package by copying and pasting or typing the following two lines of code into the console of your RStudio session. Be sure to press enter/return after each line of code.
...
To get started, let's take a peek at the data. The data file is 
...
<img style="float: right; margin: 0px 0px 20px 20px" src="img/Arbuthnot_John.jpg" alt="Portrait of John Arbuthnot by G Kneller" width="10%" height="10%">

The Arbuthnot data set refers to the work of [Dr. John Arbuthnot](https://en.wikipedia.org/wiki/John_Arbuthnot), an 18<sup>th</sup> century physician, writer, and mathematician. He was interested in the ratio of newborn boys to newborn girls, so he gathered the baptism records for children born in London for every year from 1629 to 1710. Once again, we can view the data by running the code below or by typing the name of the dataset into the console. Be careful the spelling and capitalization you use! `R` is case sensitive, so if you accidentally type `Arbuthnot` `R` will tell you that object cannot be found.
