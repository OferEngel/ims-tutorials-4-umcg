...
```{r caterpillar-run-1, eval=FALSE}
...
ranef(md_pp) |> 
  augment() |> 
  as_tibble() |> dplyr::filter(level=="cluster01") |> 
  mutate(estimate = estimate + 1.8173, lb = lb  + 1.8173, ub = ub + 1.8173 )


predictInterval(md_pp, 
        newdata = data.frame(cluster = clusters, x = 0), 
        level = .95) |> 
  add_column(
    estimate.fit = predict(md_pp, 
        newdata = data.frame(cluster = clusters, x = 0)), 
    .before = "fit"    
  ) |> mutate(diff = fit - estimate.fit)
...
md_np |> 
    predictions(
      variables = c("cluster", "y"),
      ) |> 
    as_tibble() |> select(cluster, x, y, estimate)


get_pred <- function(mdl) {
  mdl |> 
    predictions(
      by = "cluster"
      ) |> 
...
get_pred(md_np) |> 
    mutate(mdl = "no pooling") |> 
  add_row(
    get_pred(md_np_plus) |>
      mutate(mdl = "no pooling + int")
    ) |> 
  add_row(
    get_pred(md_pp) |>
      mutate(mdl = "multilevel R-Int")
    ) |>
  add_row(
    get_pred(md_np) |>
      mutate(mdl = "multilevel R-Int + R-Slope")
    ) |> 
  mutate(cluster = fct_inorder(cluster)) |> 
  ggplot(
    aes(
