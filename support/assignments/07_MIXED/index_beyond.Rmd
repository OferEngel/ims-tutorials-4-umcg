...
get_radon_vs_n_obs <- function (mdl) {
  ...
  coef_compare <- mdl  |>
    ...
    mutate(county, 
           est_me = estimate, 
           se_me = std.error, .keep = "none") |> 
    inner_join(coefs, by = "county") |> 
    select(county, est_me, est_cf, se_me, se_cf, n.obs)
  
  
  p <- coef_compare |>
    mutate(
      conf.lo = est_cf - se_cf*qnorm(.975), 
      conf.hi = est_cf + se_cf*qnorm(.975), 
      incl.mean = 
             ifelse(
               (conf.lo < mean.log.radon) & 
                 (mean.log.radon < conf.hi),
               "incl",
               "excl"
               )
      )  |>
...
# multilevel
get_radon_vs_n_obs_ml <- function (mdl) {
  set.seed(888)
  x.seq <- c(1 , seq(2, 10, 2), 10, seq(20, 100,20))
  # mdl <- m.partial_pool
  
  coef_ranef <- brms::ranef(mdl) |> as_tibble() |>
  mutate(
    county = grp,
    est_rf = condval + fixef(mdl)[1],
    se_rf = condsd,
    .keep = "none"
    ) |>
    inner_join(radon_mn_count, by = "county") 
  ...
  coef_compare <- mdl  |>
    ...
    mutate(county, 
           est_me = estimate, 
           se_me = std.error, .keep = "none") |> 
    inner_join(coef_ranef, by = "county") |> 
    select(county, est_me, est_rf, se_me, se_rf, n.obs)
  
  # coef_ranef |> 
  #   lm(se_rf ~ n.obs, data = _) |> summary()
  
  
  p <- coef_compare |>
    mutate(
      conf.lo = est_rf - se_rf*qnorm(.975), 
      conf.hi  = est_rf + se_rf*qnorm(.975), 
      incl.mean = 
             ifelse(
               (conf.lo < mean.log.radon) & 
                 (mean.log.radon < conf.hi),
...
p1 <- get_radon_vs_n_obs(m.no_pool) +
  ...
  labs(x = NULL) + 
  theme(legend.position = "none")
p2 <- get_radon_vs_n_obs_ml(m.partial_pool) +
