...
md_pp <- 
  lmer(y ~ 0 + x + (1 | cluster), df) 

# 
# In this case, the ranef and the predictions (stratified by cluster) will result in the same thing.

summary(md_pp)

p1 <- predictions(md_pp, by = "cluster") |> 
  as_tibble() |> 
  select(-s.value, -p.value) |> 
  mutate(me.est = estimate, me.est = estimate)
...
comp <- p1 |> 
  inner_join(p2, by = join_by(cluster == grp)) |> 
  select(estimate, condval, std.error, condsd)

comp |> lm(condval ~ estimate, data = _) |> 
  summary()
...
```{r test-comparing-sd.err}
# This chunk compares different methods to calculate predictions and standard errors
#
...
# Different number of obs per cluster
df_unbalanced <- add_random(uber = 1) |> 
  add_random(
    cluster = n_cluster, 
    .nested_in = "uber"
)  |> 
...
# Same number of obs per cluster
df_balanced <- add_random(uber = 1) |> 
  add_random(
    cluster = n_cluster, 
    .nested_in = "uber"
  )  |> 
...
df <- df_balanced  # equal standard errors across clusters
df <- df_unbalanced  # diff standard errors across clusters

df |> print(n=300)

# Where obs_y is the observed y (y with noise)
# exp_y is the expected y (conditional)
df <- df |>
  mutate(
    x       = rnorm(nrow(df)),
    epsilon = rnorm(nrow(df), 0, sig),
    obs_y   = a_i + b_i * x + epsilon,
    exp_y   = a_i + b_i * x
  )  |> 
  select(cluster, x, obs_y, exp_y) 


mdl <- df |> 
  lmer(y ~ 1 + (1 | cluster), data = _)

# BLOODY HELL!!! 
# merTools::shinyMer(mdl)

comp_preds <- function(df, mdl) {
  
  # This will give us the "true" cluster level intercept
  truth <- df |> 
    summarise(
      n = n(), 
      exp_y_bar = mean(exp_y), 
      obs_y_bar = mean(obs_y), 
      .by = "cluster"
    ) |> 
    mutate(level = cluster, .keep = "unused") 
  
  # Here we use marginal effects to calculate the inter
  me <- mdl |> 
    predictions(newdata = datagrid(cluster = unique)) |> 
    as_tibble() |> 
    select(cluster, estimate, std.error) |> 
    mutate(
      se_me = std.error, 
      pred_me = estimate, 
      level = cluster,
  ...
  # The coefs function give the actual prediction of the 
  # cluster level intercept, ie the global + cluster intercept
  freq <- coef(mdl)[[1]]  |>  
    rownames_to_column(var = "level") |> 
    rename(pred_freq = "(Intercept)")
  
  pred.int <- predictInterval(
    mdl, 
    newdata = data.frame(cluster = unique(df$cluster)), 
    include.resid.var = FALSE,
    ) |> 
    mutate(
      mn.fit = mean(c(upr, lwr)), 
      se = (upr - lwr) / (2 * qnorm(.975))
      ) |> as_tibble()
  
  pint <- tibble(
    level = unique(df$cluster),
    pred = predict(
      mdl, 
      newdata = data.frame(cluster = level)
      )
    ) |> 
    add_column(pred.int) |> as_tibble()
  
 
  
  
  mdl |> 
    ranef() |> augment() |> as_tibble() |> 
    mutate(fx_reff = fixef(mdl) + estimate) |> 
    select(level, estimate, std.error, fx_reff) |> 
    mutate(ranef = estimate, .keep = "unused", .after = level) |> 
    inner_join(freq, by = "level") |> 
    inner_join(me, by = "level") |> 
    inner_join(truth, by = "level") |> 
    mutate(
      real.err = (exp_y_bar - pred_freq) / (2 * qnorm(.975)), 
      prop.err = abs(std.error / real.err)
      ) |> 
    inner_join(pint, by = "level") 
  
}

# Comparing predictions...
cp <- comp_preds(df, mdl) |> 
  select(
    level, 
    fx_reff, 
    pred_freq, 
...
cor(cp |> 
      select(y_barbar, y_fixed, my_pred, .fitted))

cp |> names()

df |> 
  summarise(mean(y), .by = "cluster") |> 
  print(n = 30)


# Using the lwr and upr from the predictInterval
comp_preds(df, mdl) |> 
  ggplot(aes(
    y = my_pred, x = y_barbar, ymin = lwr, ymax = upr)) + 
  geom_pointrange() + 
  geom_abline(slope = 1) + 
  geom_smooth(mapping = aes(y_barbar, my_pred), method = "lm")


# Comparing se's
comp_preds(df, mdl) |> 
  select(y_barbar, real.sd, std.error, se_me, se)



# Sd.err is overestimated across the board!!!
comp_preds(df, mdl) |> 
  pull(prop.err) |> 
  mosaic::fav_stats()
...
predict_y |> 
  ggplot(aes(xmin = lb, xmax = ub, y = cluster, x = estimate)) + 
    geom_pointrange( ) + 
    labs(
      x = "Estimate of the intercept (Y_hat when X=0)", 
      y = NULL
      )
