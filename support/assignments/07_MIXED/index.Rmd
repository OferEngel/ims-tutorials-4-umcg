...
ggplot(finches, aes(wingl, beakh)) +
  ...
  labs(x = "wing length (mm)", y = "Beak height (mm)")

```

We can immediately see that some species (e.g., `magnirostris`) appear to be concentrated above the linear regression much more frequently than other species. This becomes clearer if we facet according to species


```{r facet-species-no-fixed, eval=TRUE}

m1 <- lm(beakh ~ wingl, finches)
ggplot(finches, aes(wingl, beakh)) +
  geom_point(alpha= 0.5, (aes(color = species))) +
  geom_abline(intercept = coef(m1)[1], slope = coef(m1)[2], color = "grey") + 
  facet_wrap(~species) + theme_clean() + 
  labs(x = "beak height (mm)", y = "residuals")
...
To deal with this, we could also use the parallel slope model, which is a fixed effects for the species, so that each species has its own intercept. The result is the following: 
...
ggplot(aes(wingl, beakh, colour = species, group = species)) +
  ...
  labs(x = "wing length (mm)", y = "beak height (mm)", title = "Fixed intercept model", subtitle = "Each species has its own intercept")
...
finches |> 
lm(beakh ~ (wingl  + species)^2, data = _) |> 
  augment() |> 
