...
lm.coef <- finches |> 
  lm(beakh ~ 1 + wingl + species, data = _) |> 
  tidy() |> mutate(term = stringr::str_replace(term, "species", "")) |> 
  mutate(term = stringr::str_replace(term, "\\(Intercept\\)", "conirostris"))

lmer(beakh ~ wingl + (1 | species), data = finches)
summary(m_lmer)
ranef(m_lmer) |> as.data.frame() |> 
mutate(total.int = condval -3.819 ) |> 
  select(grp, total.int)
