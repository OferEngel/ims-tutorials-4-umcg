...
-   `plogis(x)` you calculated the log-odds in an equation 
-   `prop.test(x, n, p)` you can use the proportion test to test either whether a proportion is equal to a hypothesized value. For example, if you sampled 100 children and found that 55 of them are girls, you can test whether the proportion in the population is 49% by running `prop.test(x = 60, n = 100, p = 0.49)`
