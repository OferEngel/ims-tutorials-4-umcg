...
M.FULL_int.ctr |> tidy() |> 
  filter(term %in% terms) |> select(term, estimate) |> 
  mutate(MEDUC = c("lower", "lower", rep(c("HS", "Some college","Uni"), times = 2)), SMOKING = c("smoke_no", "smoke_yes", rep(c("smoke_no", "smoke_yes"), each = 3))) |> pivot_wider(id_cols = MEDUC, names_from = SMOKING, values_from = estimate) |> mutate(x = 1, xend = 2) |> ggplot() + 
  ...
  labs(x = NULL, y = NULL, color = "Mother's education") + 
  theme_minimal() + 
...
6. Does the association between mother’s BMI and birthweight differ between mothers of different race/ethnicity? Modify the OPTIMAL model appropriately to be able to answer this question. Regardless of statistical significance, estimate the BMI effect at each level of mother’s race/ethnicity and make a plot illustrating the BMI effect at each level of mother’s race/ethnicity. 

```{r echo = FALSE, eval = FALSE}
M.OPT.ctr <- lm(formula = DBWT ~ DMAR + MRACEHISP + BMI + risks + PRIORLIVE + CIG_REC + PRIORDEAD, data = natality.ctr)
M.OPT_int.ctr <- lm(formula = DBWT ~ DMAR + MRACEHISP + BMI + risks + PRIORLIVE + CIG_REC + PRIORDEAD + CIG_REC*MRACEHISP, data = natality.ctr)
