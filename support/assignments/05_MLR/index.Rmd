...
5. How do the effect estimate of smoking, the standard error of the estimate, its 95% confidence interval, and its statistical significance change between the four models?
...
terms <- c("(Intercept)", "CIG_RECYes", "MEDUCHS", "MEDUCSome college", "MEDUCUni", "CIG_RECYes:MEDUCHS", "CIG_RECYes:MEDUCSome college", "CIG_RECYes:MEDUCUni")
...
M.FULL_int.ctr |> tidy() |> 
  ...
  mutate(
    MEDUC = c("lower", "lower", rep(c("HS", "Some college","Uni"), times = 2)), 
    SMOKING = c("smoke_no", "smoke_yes", rep(c("smoke_no", "smoke_yes"), each = 3))) |> 
  pivot_wider(id_cols = MEDUC, names_from = SMOKING, values_from = estimate) |> mutate(x = 1, xend = 2) |> ggplot() + 
  geom_point(aes(x = x, y = smoke_no, color = MEDUC), size = 5, alpha = .6) +
  geom_point(aes(x = xend, y = smoke_yes, color = MEDUC), size = 5) +
  geom_segment(aes(x = x, y = smoke_no, xend = xend, yend=smoke_yes, color = MEDUC), linewidth = 2) + 
  scale_x_continuous(breaks = c(1, 2), labels = c("no smoking", "smoking")) + 
...
# msummary(list(opt = M.FULL.ctr, opt_int = M.FULL_int.ctr), stars = TRUE)
...
6. Does the association between mother’s BMI and birth-weight differ between mothers of different race/ethnicity? Modify the OPTIMAL model appropriately to be able to answer this question, using the dataset with the centered variables. Regardless of statistical significance, estimate the BMI effect at each level of mother’s race/ethnicity and plot the BMI effect at each level of mother’s race/ethnicity using a dumbell plot. 

```{r dumbellplot-MRACEHISP, echo = FALSE, out.width="50%"}
...
terms <- names(M.OPT_int.ctr$coefficients)[c(1, 3, 4, 5, 6, 14, 15, 16)]

# natality.ctr$MRACEHISP |> levels()
...
M.OPT_int.ctr |> 
  ...
  select(term, estimate) |> 
  mutate(
    MRACE = rep(c("NH White", "NH Black", "NH Other", "Hispanic"), 2), 
    BMI = c(rep("BMI (mean)", each = 4), rep("BMI (+sd/2)", each = 4))) |>
  pivot_wider(id_cols = MRACE, names_from = BMI, values_from = estimate) |>
  mutate(x = 1, xend = 2) |> 
  ggplot() + 
  geom_point(aes(x = x, y = `BMI (mean)`, color = MRACE), size = 5, alpha = .6) +
  geom_point(aes(x = xend, y = `BMI (+sd/2)`, color = MRACE), size = 5) +
  geom_segment(aes(x = x, y = `BMI (mean)`, xend = xend, yend=`BMI (+sd/2)`, color = MRACE), linewidth = 2) + 
  scale_x_continuous(breaks = c(0, 1, 2, 3), labels = c(NA, "BMI (mean)", "BMI (+sd/2) ", NA)) + 
  scale_y_continuous(breaks = seq(-0.2, 0.1, 0.05)) +
...
# msummary(list(opt = M.OPT.ctr, opt_int = M.OPT_int.ctr), stars = TRUE)
# anova(M.OPT.ctr,M.OPT_int.ctr)
```


7. Does the association between mother’s BMI and birthweight depend on mother’s age? Modify the adjusted model appropriately to be able to answer this question. Regardless of statistical significance, estimate the BMI effect for mothers age 20, 30, and 40 years and make a plot illustrating the BMI effect at each of these ages. Center BMI at 25 kg/m2 and center age at 30 years. 